...
def upd_damage(damages):
    ...
    for damage in damages:
        ...
        if damage[-1] == 'M':
            update_damage.append(float(damage.strip('M')) * conversion['M'])
        if damage[-1] == 'B':
            update_damage.append(float(damage.strip('B')) * conversion['B'])
...
def dict_hurricanes(names, months, years, max_sustained_winds, areas_affected, updated_damages, deaths):
    dict = {}
    num_hurricanes = len(names)
    for i in range(num_hurricanes):
        dict[names[i]] = {'Name': names[i], 'Month': months[i], 'Year': years[i],
                          'Max sustained wind': max_sustained_winds[i], 'Areas Affected': areas_affected[i],
                          'Damage': updated_damages[i], 'Deaths': deaths[i]}
    return dict


hurricanes = dict_hurricanes(names, months, years, max_sustained_winds, areas_affected, updated_damages, deaths)
print(hurricanes)
...
def dict_hurricanes_year(names, months, years, max_sustained_winds, areas_affected, updated_damages, deaths):
    dict = {}
    num_hurricanes = len(names)
    for i in range(num_hurricanes):
        dict[years[i]] = {'Name': names[i], 'Month': months[i], 'Year': years[i],
                          'Max sustained wind': max_sustained_winds[i], 'Areas Affected': areas_affected[i],
                          'Damage': updated_damages[i], 'Deaths': deaths[i]}
    return dict


hurricanes_year = dict_hurricanes_year(names, months, years, max_sustained_winds, areas_affected, updated_damages,
                                       deaths)
...
places_affected = print(count_places(hurricanes_year))


# In[6]:


def function5(places_affected):
    dict1 = count_places(hurricanes_year)
    dict2 = {}
